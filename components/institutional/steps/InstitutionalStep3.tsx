...
import { submitInstitutionalSearch } from '@/app/actions/institutional-search';
import type { Step1Data } from './InstitutionalStep1';
import type { Step2Data } from './InstitutionalStep2';

const BUYER_TYPE_VALUES = ['privateEquity', 'familyOffice', 'strategicAcquirer', 'other'] as const;
...
interface InstitutionalStep3Props {
  step1: Step1Data;
  step2: Step2Data;
  onComplete: () => void;
}

export function InstitutionalStep3({ step1, step2, onComplete }: InstitutionalStep3Props) {
  const t = useTranslations('institutionalSearch.step3');
  ...
  const [buyerTypeOther, setBuyerTypeOther] = useState('');
  ...
  function validate(): boolean {
    ...
    if (!data.buyer_type) next.buyer_type = t('errors.buyerTypeRequired');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await submitInstitutionalSearch({
        sectors: step1.sectors.length ? step1.sectors : null,
        sector_other: step1.sector_other || null,
        ticket_size: step1.ticket_size || null,
        has_committed_capital: step1.has_committed_capital || null,
        strategy: step2.strategy.length ? step2.strategy : null,
        timeline: step2.timeline || null,
        has_acquired_before: data.has_acquired_before,
        buyer_type: data.buyer_type === 'other' && buyerTypeOther.trim()
          ? `other: ${buyerTypeOther.trim()}`
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-6">
      ...
      <Field label={t('buyerType.label')} helper={t('buyerType.helper')} error={errors.buyer_type}>
        <div className="flex flex-col gap-3">
          {BUYER_TYPE_VALUES.map((key) => (
            <RadioCard
              key={key}
              label={t(`buyerType.${key}`)}
              name="buyer_type"
              selected={data.buyer_type === key}
              hasError={!!errors.buyer_type}
              onChange={() => set('buyer_type', key)}
            />
          ))}
        </div>
        {data.buyer_type === 'other' && (
          <Input
            type="text"
            value={buyerTypeOther}
            onChange={(e) => setBuyerTypeOther(e.target.value)}
            placeholder={t('buyerType.otherPlaceholder')}
            className="mt-2"
          />
        )}
      </Field>
