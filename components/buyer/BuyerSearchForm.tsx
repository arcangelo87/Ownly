...
import { BuyerStepIndicator } from './BuyerStepIndicator';
  ...
  return (
    <div className={`flex flex-col md:flex-row${modal ? '' : ' min-h-screen'}`}>
      ...
      <main className="flex-1 px-5 py-10 md:px-16 md:py-14" style={{ maxWidth: '680px' }}>
        ...
        {currentStep === 1 && (
          <BuyerStep1 onComplete={handleStep1Complete} />
        )}
        {currentStep === 2 && (
          <BuyerStep2 onComplete={handleStep2Complete} />
        )}
...
function SuccessScreen() {
  const t = useTranslations('buyerSearch.success');
  return (
